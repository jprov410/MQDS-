...
{
    std::srand(static_cast<unsigned int>((unsigned int) (5 * mype + 1 ) * std::time(nullptr) + 1673 * mype));
}
...
double MQDS::Random::gaussian_random_number(double const &mean, double const &std_dev) {
    //double m_gauss;
...
//              * std::cos( 2.0 * MQDS::Constants::pi * MQDS::Random::uniform_random_number(0.0, 1.0));
    ...
    return std::sqrt(-2.0 * std::log(MQDS::Random::uniform_random_number(0.0, 1.0)))
           * std::cos( 2.0 * MQDS::Constants::pi * MQDS::Random::uniform_random_number(0.0, 1.0))
           *std_dev + mean;
}
